...
run_demo_app <- function(
    ...
    config_file = system.file("golem-config.yml", package = "GermlineVarDB"),
    app_title = "My Demo Database",
    ...
) {
  if(browser == TRUE){
    config <- paste0(config,"_browser")
    download_genome_reference(browser_server_path = system.file("extdata/testdata/JBrowseR/", package = "GermlineVarDB"),
                              "hg19")
  }
  with_golem_options(
    app = shinyApp(
